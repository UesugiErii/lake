...
class ProximalPolicyOptimization():
    ...
    def _build_net(self):
        ...
        self.L = tf.reduce_sum(
            tf.multiply(
                tf.divide(self.action_prob, self.select_old_act_pro),
                self.tf_adv
            )
        )  /  tf.cast(tf.shape(self.tf_a)[0], tf.float32)

        self.penalty = tf.reduce_sum(
            tf.multiply(self.all_act_prob,
                        tf.log(
                            tf.divide(
                                self.all_act_prob,
                                self.tf_old_act_pro
        ...
        self.p_loss = - (self.L - self.tf_beta * self.penalty)

        self.p_opt = tf.train.AdamOptimizer(self.lr).minimize(self.p_loss,var_list=self.policy_parameter)


        self.multiply_beta = tf.assign(self.tf_beta,2*self.tf_beta)
        self.divide_beta = tf.assign(self.tf_beta,self.tf_beta/2)
    ...
    def learn(self):
        ...
        DKL,_ = self.sess.run([self.penalty,self.p_opt,],feed_dict={
            self.tf_s:s,
        ...
        if DKL >= 1.5 * self.delta:
            self.sess.run(self.multiply_beta)
        else:
            self.sess.run(self.divide_beta)
