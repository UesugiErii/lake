...
class NaturalPolicyGradient:
    ...
    def _build_net(self):
        ...
        self.learning_rate = tf.sqrt(
            tf.divide(0.001, tf.matmul(tf.transpose(self.g), self.F_inv_g)))

        update = tf.multiply(self.learning_rate, self.F_inv_g)
        self.update = tf.reshape(update, (4, 2))

        self.opt = tf.assign_add(self.policy_parameter[0], self.update)
...
def main():
    ...
    env = env.unwrapped
    agent = NaturalPolicyGradient(env.action_space.n, env.observation_space.shape[0])
