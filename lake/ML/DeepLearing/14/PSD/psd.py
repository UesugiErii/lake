...
h = tf.Variable(tf.random.normal([batch_size, hidden_size], stddev=0.01), trainable=True)
...
def train_step2(images, labels):
    ...
    optimizer2.apply_gradients(zip([gradients], [h]))
...
print(np.max(encoder(img).numpy))
reimg = reimg.numpy()
from matplotlib import pyplot as plt

for i in range(n):
    img.resize((batch_size, 28, 28, 1))
    reimg.resize((batch_size, 28, 28, 1))
