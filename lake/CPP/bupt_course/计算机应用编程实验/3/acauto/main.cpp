...
AC::AC() {
    ...
    pattern_offset = (int **) malloc(SZ * sizeof(int *));
    for (int i = 0; i < SZ; i++)
        pattern_offset[i] = (int *) malloc(3 * sizeof(int));  // transfer
...
void AC::query(char *t, int &str_len) {
    ...
    for (int i = 0; t[i]; i++) {
        ...
        for (int j = u; j; j = fail[j]) {
            id = idx[j];
            if (id) {
                if(cnt[id] < 3) pattern_offset[id][cnt[id]] = string_offset + i - pattern_len[id] + 1;
                cnt[id]++;
            }
        }
    }
...
//    }
    ...
    int total_appear = 0;  // 多少种模式串出现过
    int total_count = 0;  // 所有模式串总出现次数
    ...
    for (int i = 1; i <= ac.id; i++) {
        ...
        for (int j = 0; j < ac.cnt[id] and j < 3; j++) {
            fputc(' ', fp_result);
            fputs(to_string(ac.pattern_offset[id][j]).c_str(), fp_result);
        }
        total_count += ac.cnt[id];
        if(ac.cnt[id]) total_appear += 1;
        fputc('\n', fp_result);
